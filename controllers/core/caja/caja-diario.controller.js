...
const cerrarCaja = async (req, res = response) => {
  try {
    ...
    // asignar valores
    let monto_doscientos_soles =
      req.body.cantidad_doscientos_soles_cierre * 200;
    ...
    let monto_cincuenta_centimos =
      (req.body.cantidad_cincuenta_centimos_cierre * 5) / 10;
    let monto_veinte_centimos =
      (req.body.cantidad_veinte_centimos_cierre * 2) / 10;
    ...
    let monto_total =
      monto_doscientos_soles +
      monto_cien_soles +
      monto_cincuenta_soles +
      monto_veinte_soles +
      monto_diez_soles +
      monto_cinco_soles +
      monto_dos_soles +
      monto_un_sol +
      monto_cincuenta_centimos +
      monto_veinte_centimos +
      monto_diez_centimos;
    ...
    (modelo.cierre = {
      ...
      cantidad_doscientos_soles_cierre:
        req.body.cantidad_doscientos_soles_cierre,
      ...
      cantidad_cincuenta_centimos_cierre:
        req.body.cantidad_cincuenta_centimos_cierre,
...
const cargarCaja = async (req, res) => {
  const id_usuario_sesion = /* "5f8236bedd1aaa4dc4109589"; */ req.header(
    "id_usuario_sesion"
  );
  const ip = /* "192.168.0.10"; */ requestIp
    .getClientIp(req)
    .replace("::ffff:", "");
  ...
  try {
    ...
    const cant_operaciones = await Operaciones.find({
      es_vigente: true,
      es_borrado: false,
      "recibo.estado": { $in: ["Vigente", "Previgente"]},
...
const listarCajasPorFecha = async (req, res) => {
  ...
  try {
    const cajasFecha = await CajaDiario.findOne(
