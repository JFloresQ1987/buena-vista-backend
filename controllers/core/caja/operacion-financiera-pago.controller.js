    ...
    for (let i = 0; i < lista.length; i++) {
      ...
      for (let j = 0; j < lista[i].pagos.length; j++) {
        ...
        monto_ahorro_inicial_pagado +=
          lista[i].pagos[j].ahorros.monto_ahorro_inicial || 0;
        monto_ahorro_voluntario_pagado +=
          lista[i].pagos[j].ahorros.monto_ahorro_voluntario || 0;
        monto_ahorro_programado_pagado +=
          lista[i].pagos[j].ahorros.monto_ahorro_programado || 0;
        monto_amortizacion_capital_pagado +=
          lista[i].pagos[j].ingresos.monto_amortizacion_capital || 0;
      ...
      lista[
        i
      ].ahorros.monto_ahorro_voluntario -= monto_ahorro_voluntario_pagado;
      lista[
        i
      ].ahorros.monto_ahorro_programado -= monto_ahorro_programado_pagado;
      lista[
        i
      ].ingresos.monto_amortizacion_capital -= monto_amortizacion_capital_pagado;
  ...
  try {
    ...
    for (let i = 0; i < cuotas.length; i++) {
      ...
      let monto_ahorro_inicial_a_pagar =
        cuota.ahorros.monto_ahorro_inicial - monto_ahorro_inicial_pagado;
      ...
      let monto_ahorro_programado_a_pagar =
        cuota.ahorros.monto_ahorro_programado - monto_ahorro_programado_pagado;
      let monto_amortizacion_capital_a_pagar =
        cuota.ingresos.monto_amortizacion_capital -
        monto_amortizacion_capital_pagado;
      let monto_interes_a_pagar =
        cuota.ingresos.monto_interes - monto_interes_pagado;
...
const registrarIngresoEgreso = async (req, res = response) => {
  ...
  const ip = "192.168.13.1"; //requestIp.getClientIp(req).replace("::ffff:", "");
  try {
    const caja = await Caja.findOne({
      ip: ip,
      usuario: id_usuario_sesion,
    ...
    let dato_recibo = {
      serie_recibo: "001",
      numero_recibo: es_ingreso ? "I-00000001" : "E-00000001",
      fecha_recibo: now.format("DD/MM/YYYY hh:mm:ss a"),
    };
    ...
    dato_recibo.numero_recibo =
      (es_ingreso ? "I-" : "E-") +
      correlativo_recibo.toString().padStart(8, "00000000");

    modelo.es_ingreso = es_ingreso;
    ...
    let recibo = [
      ...
      [
        {
          content: "DNI: " + documento_identidad_responsable,
          colSpan: 3,
          styles: { halign: "left" },
          // styles: { halign: 'center', fillColor: [22, 160, 133] },
        },
      ...
      [
        {
          content: "Socio: " + nombres_apellidos_responsable,
          colSpan: 3,
          styles: { halign: "left" },
          // styles: { halign: 'center', fillColor: [22, 160, 133] },
        },
